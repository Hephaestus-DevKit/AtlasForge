...
pub fn check_permission(
    ...
    if let Some(path_str) = path_candidate {
        ...
        if tool_name.starts_with("fs.") {
            crate::security::authorize_read(path, roots)?;
        } else if crate::security::authorize_path(path, roots).is_none() {
            return Err(format!(
...
pub fn invoke_tool(
    ...
    // Execute the tool
    let mut result = execute_tool(tool_name, input, &roots);
    // Tool results can be model-facing. Return the same redacted value that is
    // persisted to the audit database, never the raw executor output.
    result.output = crate::ai_provider::redact_secrets(&result.output);
    result.error = result
        .error
        .as_deref()
...
fn execute_tool(tool_name: &str, input: &serde_json::Value, roots: &[WorkspaceRoot]) -> ToolResult {
    match tool_name {
            ...
            match std::fs::read_dir(p) {
                Ok(read_dir) => {
                    let mut entries: Vec<String> = read_dir
                        .filter_map(|e| e.ok())
                        .filter(|entry| {
                            crate::security::authorize_read(&entry.path(), roots).is_ok()
                        })
        ...
        "fs.read" => {
            ...
            let read = (|| -> Result<(String, bool), String> {
                use std::io::Read;
                let file = std::fs::File::open(path).map_err(|error| error.to_string())?;
                let mut bytes = Vec::with_capacity(max_len + 1);
                file.take((max_len + 1) as u64)
        ...
        "git.status" | "git.diff" => {
            ...
            let args: Vec<&str> = if tool_name == "git.status" {
                vec!["-c", "core.fsmonitor=false", "status", "--porcelain"]
            } else {
                vec![
                    "-c",
...
mod tests {
    ...
    fn roadmap_tools_are_not_exposed_as_implemented() {
        ...
        assert!(!is_implemented("shell.verify"));
    ...
    #[test]
    fn fs_tools_block_sensitive_paths_and_filter_listings() {
        let temp = tempfile::tempdir().unwrap();
        fs::write(temp.path().join(".env"), "TOKEN=short").unwrap();
        fs::write(temp.path().join("readme.txt"), "safe").unwrap();
        let roots = vec![root_for(temp.path())];
