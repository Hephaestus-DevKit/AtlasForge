...
| Text indexing | Implemented | Sensitive paths are excluded, text is redacted, stale files are removed, and unchanged files are skipped by content hash and index version. | Indexer tests | File watching is optional at current scale. |
